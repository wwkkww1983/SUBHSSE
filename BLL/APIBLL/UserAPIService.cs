...
namespace BLL
{
    public static class  UserAPIService
    {
       ...
       /// <returns></returns>
        ...
        {
            var getUser = Funs.DB.Sys_User.FirstOrDefault(x => (x.Account == userInfo.Account || x.Telephone == userInfo.Telephone) && x.IsPost == true && x.Password == Funs.EncryptionPassword(userInfo.Password));
            return ObjectMapperManager.DefaultInstance.GetMapper<Model.Sys_User, Model.UserItem>().Map(getUser);
        }
        ...
        /// <returns></returns>
        public static List<Model.UserItem> UserLogOn2(string account, string password)
        {
