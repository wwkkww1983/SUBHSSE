...
namespace BLL
{
    public static class SafetyDataPlanService
    {
        ...
        {
            Model.SUBHSSEDB db = Funs.DB;
            var safetyDataPlan = from x in db.SafetyData_SafetyDataPlan where x.ProjectId == projectId select x;
        ...
        {
            Model.SUBHSSEDB db = Funs.DB;
            var safetyDataPlan = from x in db.SafetyData_SafetyDataPlan where x.ProjectId == projectId && x.SafetyDataId == safetyDataId select x;
            if (safetyDataPlan.Count() > 0)
            {
                db.SafetyData_SafetyDataPlan.DeleteAllOnSubmit(safetyDataPlan);
                db.SubmitChanges();
        ...
        {
            Model.SUBHSSEDB db = Funs.DB;
            var safetyDataPlan = from x in db.SafetyData_SafetyDataPlan where x.ProjectId == projectId && x.CheckDate > projectDate select x;
            if (safetyDataPlan.Count() > 0)
            {
                db.SafetyData_SafetyDataPlan.DeleteAllOnSubmit(safetyDataPlan);
                db.SubmitChanges();
        ...
        /// </summary>
        /// <param name="safetyDataPlanId"></param>
        public static void GetSafetyDataPlanByProjectInfo(string projectId, string safetyDataId)
        {           
            Model.SUBHSSEDB db = Funs.DB;
            ...
            if (!isDelete(projectId, safetyDataId))
            {
                ...
                if (safetyDataEnd.Count() > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(projectId))
                    {
                        ...
                        if (project != null)
                        {                            
                            GetSafetyDataPlanMethod(project, safetyDataEnd);
                        }
                    }
                    else
                    {
                        ///取竣工时间大于当前时间 项目
                        var projects = from x in db.Base_Project where x.EndDate >= System.DateTime.Now select x;
                        if (projects.Count() > 0)
                        {
                            foreach (var item in projects)
                            {                                
                                GetSafetyDataPlanMethod(item, safetyDataEnd);
                            }
                        }
                    }
                }
        ...
        public static void GetSafetyDataPlanMethod(Model.Base_Project project, List<Model.SafetyData_SafetyData> safetyDataEnd)
        {
            ////第一步 判断是否存在此项目的计划表
            ...
            {
                if (!isDelete(projectId, item.SafetyDataId))
                {                    
                    var safetyDataPlan = db.SafetyData_SafetyDataPlan.FirstOrDefault(x => x.ProjectId == project.ProjectId && x.SafetyDataId == item.SafetyDataId);
                        ...
                        if (item.CheckType == BLL.Const.SafetyDataCheckType_1) /// 月报
                        {
                            ...
                            if (checkDate.HasValue && checkDate <= endDate && checkDate >= startDatep)
                            {
                                Model.SafetyData_SafetyDataPlan newSafetyDataPlan = new Model.SafetyData_SafetyDataPlan
                                {
                                    ...
                                    Remark = item.Remark,
                                    CheckDate = checkDate,
                                    RealStartDate = checkDate.Value.AddMonths(-1), ///月报开始日期
                                    RealEndDate = checkDate.Value
                                };
                                ///结束日期
                                db.SafetyData_SafetyDataPlan.InsertOnSubmit(newSafetyDataPlan);
                                db.SubmitChanges();
                            ...
                            if ((month == 3 + monthValue) || (month == 6 + monthValue) || (month == 9 + monthValue) || (month == monthValue) || (month == 12 && monthValue == 0)) ///考核季度时间
                            {
                                ...
                                if (checkDate.HasValue && checkDate <= endDate && checkDate >= startDatep)
                                {
                                    Model.SafetyData_SafetyDataPlan newSafetyDataPlan = new Model.SafetyData_SafetyDataPlan
                                    {
                                        ...
                                        Remark = item.Remark,
                                        CheckDate = checkDate,
                                        RealStartDate = checkDate.Value.AddMonths(-3), ///开始日期
                                        RealEndDate = checkDate.Value
                                    };
                                    ///结束日期
                                    db.SafetyData_SafetyDataPlan.InsertOnSubmit(newSafetyDataPlan);
                            ...
                            if (startDate.AddMonths(i).Month == monthValue) ///定时月份
                            {
                                ...
                                if (checkDate.HasValue && checkDate <= endDate && checkDate >= startDatep)
                                {
                                    Model.SafetyData_SafetyDataPlan newSafetyDataPlan = new Model.SafetyData_SafetyDataPlan
                                    {
                                        ...
                                        Remark = item.Remark,
                                        CheckDate = checkDate,
                                        RealStartDate = checkDate.Value.AddMonths(-12), ///开始日期
                                        RealEndDate = checkDate.Value
                                    };
                                    ///结束日期
                                    db.SafetyData_SafetyDataPlan.InsertOnSubmit(newSafetyDataPlan);
                            ...
                            if (startDate.AddMonths(i).Month == monthValue || startDate.AddMonths(i).Month == monthValue + 6)
                            {
                                ...
                                if (checkDate.HasValue && checkDate <= endDate && checkDate >= startDatep)
                                {
                                    Model.SafetyData_SafetyDataPlan newSafetyDataPlan = new Model.SafetyData_SafetyDataPlan
                                    {
                                        ...
                                        Remark = item.Remark,
                                        CheckDate = checkDate,
                                        RealStartDate = checkDate.Value.AddMonths(-6), ///开始日期
                                        RealEndDate = checkDate.Value
                                    };
                                    ///结束日期
                                    db.SafetyData_SafetyDataPlan.InsertOnSubmit(newSafetyDataPlan);
        ...
        public static void DeleteSafetyDataPlanDelete(string projectId, string safetyDataId)
        {
            ...
            GetSafetyDataPlanByProjectInfo(projectId, safetyDataId);
