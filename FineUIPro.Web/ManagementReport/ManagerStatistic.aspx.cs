    ...
    public partial class ManagerStatistic : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                this.txtEndTime.Text = string.Format("{0:yyyy-MM}", DateTime.Now);
            }
        }
        #endregion
        ...
        protected void BtnStatistic_Click(object sender, EventArgs e)
        {
            ...
            startTime = Convert.ToDateTime(this.txtStartTime.Text.Trim() + "-" + "1");
            endTime = Convert.ToDateTime(this.txtEndTime.Text.Trim() + "-" + "1").AddMonths(1);
            List<Model.Manager_MonthReportB> monthReports = BLL.MonthReportBService.GetMonthReportsByStartAndEndTime(startTime, endTime);
            // 获取项目最近的月报
            DateTime lastMonth = Convert.ToDateTime(this.txtEndTime.Text.Trim() + "-" + "1");
            List<Model.Manager_MonthReportB> lastMonthReports = BLL.MonthReportBService.GetMonthReportsByMonths(lastMonth);
            //获取数据
        ...
        private void GetProjectStation(string state)
        {
            ...
            Model.Manager_HeadMonthReportB LastHeadMonthReport = BLL.HeadMonthReportBService.GetLastHeadMonthReportByMonths(endTime.AddMonths(-1));
            ...
            foreach (Model.Base_Project project in allProjects)
            {
                ...
                // 当月的月报
                List<Model.Manager_MonthReportB> monthReports = BLL.MonthReportBService.GetMonthReportsByStartAndEndTimeAndProjectId(startTime, endTime, project.ProjectId);
                // 最近的月报
            ...
            var q1 = (from x in Funs.DB.Manager_AccidentSortB
                      ...
                      && y.Months >= startTime && y.Months < endTime
