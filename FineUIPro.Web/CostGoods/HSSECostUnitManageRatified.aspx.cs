...
{
    public partial class HSSECostUnitManageRatified : PageBase
    {
        #region 项目主键
        /// <summary>
        /// 项目主键
        ...
        private void BindGrid()
        {
            string strSql = @" SELECT UnitManage.HSSECostUnitManageId,UnitManage.HSSECostManageId,CostManage.Month,CostManage.Code,Unit.UnitCode,Unit.UnitName,SysUser.UserName AS CompileMan,UnitManage.CompileDate"
                        + @" ,(UnitManage.RatifiedCostA1+UnitManage.RatifiedCostA2+UnitManage.RatifiedCostA3+UnitManage.RatifiedCostA4+UnitManage.RatifiedCostA5+UnitManage.RatifiedCostA6+UnitManage.RatifiedCostA7+UnitManage.RatifiedCostA8) AS CostAsum"
                        ...
                        + @" WHEN UnitManage.States = '" + BLL.Const.State_2 + "' THEN '核定完成'"
                        + @" ELSE '待[' + OperateUser.UserName + ']办理' END) AS FlowOperateName"
                        + @" FROM CostGoods_HSSECostUnitManage AS UnitManage"
                        + @" LEFT JOIN Sys_FlowOperate AS FlowOperate ON UnitManage.HSSECostUnitManageId +'#3'=FlowOperate.DataId AND FlowOperate.IsClosed <> 1"
            ...
            if (this.rbStateType.SelectedValue == "3")
            {
            ...
            {
                strSql += " AND UnitManage.StateType != 1 AND UnitManage.StateType != 2 AND UnitManage.StateType IS NOT NULL";
            }
        ...
        private void EditData()
        {
            ...
            if (unitManage != null)
            {
                if (!this.btnMenuEdit.Hidden && unitManage.StateType == "3")   ////双击事件 编辑权限有：编辑页面，无：查看页面 或者状态是完成时查看页面
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("HSSECostUnitManageRatifiedEdit.aspx?HSSECostUnitManageId={0}", id, "编辑 - ")));
                }
                else
                {
        ...
        private void GetButtonPower()
        {
            ...
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.LoginProjectId, this.CurrUser.UserId, BLL.Const.ProjectHSSECostUnitManageRatifiedMenuId);
