...
{
    public partial class UnexpectedEnvironmentalEdit : PageBase
    {
        #region 定义项
        /// <summary>
        /// 主键
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {               
                ...
                if (!string.IsNullOrEmpty(this.FileId))
                {
                    ...
                    {
                        this.ProjectId = UnexpectedEnvironmental.ProjectId;
                        ...
                        if (!string.IsNullOrEmpty(UnexpectedEnvironmental.CompileMan))
                        {
                            this.drpCompileMan.SelectedValue = UnexpectedEnvironmental.CompileMan;
                        }
                ...
                {
                    ////自动生成编码
                    this.txtFileCode.Text = BLL.CodeRecordsService.ReturnCodeByMenuIdProjectId(BLL.Const.UnexpectedEnvironmentalMenuId, this.ProjectId, this.CurrUser.UnitId);
        ...
        {
            Model.Environmental_UnexpectedEnvironmental UnexpectedEnvironmental = new Model.Environmental_UnexpectedEnvironmental
            {
            ...
            {
                UnexpectedEnvironmental.CompileMan = this.drpCompileMan.SelectedValue;
            }
            UnexpectedEnvironmental.CompileDate = Funs.GetNewDateTime(this.txtCompileDate.Text.Trim());
            UnexpectedEnvironmental.FileContent = HttpUtility.HtmlEncode(this.txtFileContent.Text);
            ////单据状态
            UnexpectedEnvironmental.States = BLL.Const.State_0;
            if (type == BLL.Const.BtnSubmit)
            {
                UnexpectedEnvironmental.States = this.ctlAuditFlow.NextStep;
            }
            if (!string.IsNullOrEmpty(this.FileId))
            {
            ...
            {
                this.FileId = SQLHelper.GetNewID(typeof(Model.Environmental_UnexpectedEnvironmental));
                UnexpectedEnvironmental.FileId = this.FileId;
        ...
        protected void btnAttachUrl_Click(object sender, EventArgs e)
        {
            ...
            PageContext.RegisterStartupScript(WindowAtt.GetShowReference(String.Format("../AttachFile/webuploader.aspx?toKeyId={0}&path=FileUpload/UnexpectedEnvironmentalAttachUrl&menuId={1}", FileId, BLL.Const.UnexpectedEnvironmentalMenuId)));
        }
        #endregion
    }
}