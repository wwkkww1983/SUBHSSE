...
{
    public partial class CompanyEnvironmental : PageBase
    {
        #region 加载页面
        /// <summary>
        /// 加载页面
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                //权限设置
                this.GetButtonPower();
                btnNew.OnClientClick = Window1.GetShowReference("EnvironmentalEdit.aspx?IsCompany=True") + "return false;";
        ...
        private void BindGrid()
        {
            string strSql = @"SELECT Environmental.EnvironmentalId,Environmental.EType,Environmental.ActivePoint,Environmental.EnvironmentalFactors,Environmental.AValue,Environmental.BValue,Environmental.CValue,Environmental.DValue,Environmental.EValue,Environmental.ZValue,Environmental.SmallType,Environmental.IsImportant,Environmental.Code,Environmental.ControlMeasures,Environmental.Remark"
                ...
                + @" WHERE (Environmental.IsCompany ='True') ";
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.LoginProjectId, this.CurrUser.UserId, BLL.Const.CompanyEnvironmentalMenuId);
        ...
        {
            PageContext.RegisterStartupScript(Window2.GetShowReference(String.Format("EnvironmentalIn.aspx?IsCompany=True", "导入 - ")));
        }
        #endregion
        ...
        protected void btnOut_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + System.Web.HttpUtility.UrlEncode("公司环境危险源清单" + filename, Encoding.UTF8) + ".xls");
