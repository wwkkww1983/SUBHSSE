...
namespace FineUIPro.Web.HiddenInspection
{
    public partial class ManagerPerformance : PageBase
    {
        ...
        private void InitGrid()
        {
            ...
            bf.HeaderText = "巡检人员姓名";
            bf.Width = System.Web.UI.WebControls.Unit.Pixel(120);
        ...
        private void BindGrid()
        {
            ...
            numList.Add(0);
            var checkMan = (from x in hazardRegisters select x.CheckManName).Distinct().ToList();
            foreach (var item in checkMan)
            {
                DataRow rowTime = table.NewRow();
                rowTime["Name"] = item;
                rowTime["Total"] = hazardRegisters.Where(x => x.CheckManName == item).Count();
                int i = 0;
                foreach (var hazardRegisterType in hazardRegisterTypes)
                {
                    rowTime[hazardRegisterType.TypeCode] = hazardRegisters.Where(x => x.CheckManName == item && x.RegisterTypesId == hazardRegisterType.RegisterTypesId).Count();
                    numList[i] += hazardRegisters.Where(x => x.CheckManName == item && x.RegisterTypesId == hazardRegisterType.RegisterTypesId).Count();
                    i++;
                }
                //专项检查
                rowTime["Special"] = hazardRegisters.Where(x => x.ProblemTypes == "4").Count();
                numList[i] = hazardRegisters.Where(x => x.ProblemTypes == "4").Count();
        ...
        protected void btnOut_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + System.Web.HttpUtility.UrlEncode("管理人员履职情况" + filename, System.Text.Encoding.UTF8) + ".xls");
