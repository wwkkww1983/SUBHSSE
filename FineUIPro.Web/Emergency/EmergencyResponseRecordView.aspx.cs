...
{
    public partial class EmergencyResponseRecordView : PageBase
    {
        #region 定义项
        /// <summary>
        /// 主键
            ...
            {               
                ...
                if (!string.IsNullOrEmpty(this.FileId))
                {
                    ...
                    if (EmergencyResponseRecord != null)
                    {
                        ...
                        this.txtUnit.Text = BLL.UnitService.GetUnitNameByUnitId(EmergencyResponseRecord.UnitId);
                        this.txtCompileDate.Text = string.Format("{0:yyyy-MM-dd}", EmergencyResponseRecord.CompileDate);
                        this.drpCompileMan.Text = BLL.UserService.GetUserNameByUserId(EmergencyResponseRecord.CompileMan);
        ...
        protected void btnAttachUrl_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(this.FileId))
            {
                PageContext.RegisterStartupScript(WindowAtt.GetShowReference(String.Format("../AttachFile/webuploader.aspx?toKeyId={0}&path=FileUpload/EmergencyResponseRecordAttachUrl&menuId={1}&type=-1", FileId, BLL.Const.ProjectEmergencyResponseRecordMenuId)));
            }            
        }
        #endregion
    }
