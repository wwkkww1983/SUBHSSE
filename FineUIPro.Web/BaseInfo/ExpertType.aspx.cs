...
{
    public partial class ExpertType : PageBase
    {
        #region 加载
        /// <summary>
        /// 加载页面
        ...
        {
            var q = from x in Funs.DB.Base_ExpertType orderby x.ExpertTypeCode select x;
        ...
        /// </summary>
        /// <returns></returns>
        private List<Model.Base_ExpertType> GetPagedDataTable(int pageIndex, int pageSize)
        {
            List<Model.Base_ExpertType> source = (from x in BLL.Funs.DB.Base_ExpertType orderby x.ExpertTypeCode select x).ToList();
            List<Model.Base_ExpertType> paged = new List<Model.Base_ExpertType>();
        ...
        private bool judgementDelete(string id, bool isShow)
        {
            ...
            if (Funs.DB.Technique_Expert.FirstOrDefault(x => x.ExpertTypeId == id) != null)
            {
                content = "该专家类别已在【安全专家】中使用，不能删除！";
            }
        ...
        protected void btnMenuEdit_Click(object sender, EventArgs e)
        {
            ...
            var expertType = BLL.ExpertTypeService.GetExpertTypeById(Id);
            if (expertType != null)
            {
                this.txtExpertTypeCode.Text = expertType.ExpertTypeCode;
                this.txtExpertTypeName.Text = expertType.ExpertTypeName;
                this.txtRemark.Text = expertType.Remark;
                hfFormID.Text = Id;
        ...
        {
            var q = Funs.DB.Base_ExpertType.FirstOrDefault(x => x.ExpertTypeCode == this.txtExpertTypeCode.Text.Trim() && (x.ExpertTypeId != hfFormID.Text || (hfFormID.Text == null && x.ExpertTypeId != null)));
            if (q != null)
            {
                ShowNotify("输入的类别编号已存在！", MessageBoxIcon.Warning);
            }

            var q2 = Funs.DB.Base_ExpertType.FirstOrDefault(x => x.ExpertTypeName == this.txtExpertTypeName.Text.Trim() && (x.ExpertTypeId != hfFormID.Text || (hfFormID.Text == null && x.ExpertTypeId != null)));
            if (q2 != null)
            {
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.LoginProjectId, this.CurrUser.UserId, BLL.Const.ExpertTypeMenuId);
