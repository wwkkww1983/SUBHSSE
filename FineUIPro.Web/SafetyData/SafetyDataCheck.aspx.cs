...
{
    public partial class SafetyDataCheck : PageBase
    {
        #region 加载
        /// <summary>
        /// 加载页面
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ////权限按钮方法
                this.GetButtonPower();
                this.btnNew.OnClientClick = Window1.GetShowReference("SafetyDataCheckEdit.aspx") + "return false;";
        ...
        {
            string strSql = @"SELECT SafetyDataCheckId,Code,Title,StartDate,EndDate,CompileDate,Users.UserName AS CompileManName"
                          + @" FROM SafetyData_SafetyDataCheck"
                          + @" LEFT JOIN Sys_User AS Users ON  Users.UserId = CompileMan WHERE 1=1  ";                         
            ...
            if (safetyDataCheckItem.Count() > 0)
            {
                for (int i = 0; i < Grid1.Rows.Count; i++)
                {
                    ...
                    {
                        Grid1.Rows[i].RowCssClass = "LightBlue";
                    }
                }
            }
        }
        ...
        private void EditData()
        {
            ...
            if (SafetyData != null)
            {
                ...
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("SafetyDataCheckEdit.aspx?SafetyDataCheckId={0}&value=0", id, "查看 - ")));                    
                }
                else
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("SafetyDataCheckEdit.aspx?SafetyDataCheckId={0}", id, "编辑 - ")));
                }
            }            
        }
        #endregion
        ...
        protected void btnMenuDelete_Click(object sender, EventArgs e)
        {
            ...
            {               
                foreach (int rowIndex in Grid1.SelectedRowIndexArray)
                {
                    ...
                    BLL.LogService.AddLogDataId(this.CurrUser.LoginProjectId, this.CurrUser.UserId, "删除企业安全管理资料考核", rowID);
                    ////需要添加 删除明细方法                        
                    BLL.SafetyDataCheckService.DeleteSafetyDataCheckById(rowID);
                }
        ...
        protected void btnOut_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + System.Web.HttpUtility.UrlEncode("安全管理资料考核" + filename, System.Text.Encoding.UTF8) + ".xls");
