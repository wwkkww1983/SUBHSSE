    ...
    public partial class FireActivities : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ////权限按钮方法
                this.GetButtonPower();
                this.btnNew.OnClientClick = Window1.GetShowReference("FireActivitiesEdit.aspx") + "return false;";
        ...
        private void EditData()
        {
            ...
            var meeting = BLL.FireActivitiesService.GetFireActivitiesById(id);
            if (meeting != null)
            {
                if (this.btnMenuEdit.Hidden)   ////双击事件 编辑权限有：编辑页面，无：查看页面 或者状态是完成时查看页面
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("FireActivitiesView.aspx?FireActivitiesId={0}", id, "查看 - ")));                    
                }
                else
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("FireActivitiesEdit.aspx?FireActivitiesId={0}", id, "编辑 - ")));
                }
            }            
        }
        #endregion
        ...
        private void GetButtonPower()
        {
            ...
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.LoginProjectId, this.CurrUser.UserId, BLL.Const.ProjectFireActivitiesMenuId);
...
#pragma warning disable CS0108 // “FireActivities.GetGridTableHtml(Grid)”隐藏继承的成员“PageBase.GetGridTableHtml(Grid)”。如果是有意隐藏，请使用关键字 new。
        /// <summary>
        /// 导出方法
        /// </summary>
...
#pragma warning restore CS0108 // “FireActivities.GetGridTableHtml(Grid)”隐藏继承的成员“PageBase.GetGridTableHtml(Grid)”。如果是有意隐藏，请使用关键字 new。
        {
